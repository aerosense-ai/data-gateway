...
def waitTillSetComplete(type, t):   #timestamp in 1/(2**16) s
    if type == "Mics" or type == "Baros" or type == "Analog":
        ...
        if t != currentTimestamp[type] and currentTimestamp[type] != 0:
            ...
            if abs(idealNewTimestamp - currentTimestamp[type]) > MAX_TIMESTAMP_SLACK*(2**16):  #If at least one set (= one packet per mic/baro group) of packets was lost
                if prevIdealTimestamp[type] != 0:
                    print("Lost set of " + type + " packets: " + str((currentTimestamp[type] - idealNewTimestamp)/(2**16)*1000) + "ms gap")
                else:
        ...
        if currentTimestamp[type] != 0:
            per = period[type]
            if prevIdealTimestamp[type] != 0:   #If there is a previous timestamp, calculate the actual sampling period from the difference to the current timestamp
                per = (currentTimestamp[type] - prevIdealTimestamp[type])/samplesPerPacket[type]/(2**16)
                if abs(per - period[type])/period[type] < MAX_PERIOD_DRIFT:   #If the calculated period is reasonable, accept it. If not, most likely a packet got lost
                    period[type] = per
                else:
                    print("Lost " + type + " packet: " + str((currentTimestamp[type] - prevIdealTimestamp[type])/(2**16)*1000) + "ms gap")
            else:
    ...
    if handles[type].startswith("Baro group"):
        ...
        for i in range(BAROS_SAMPLES_PER_PACKET):
            for j in range(BAROS_GROUP_SIZE):
                data["Baros"][baroGroupNum*BAROS_GROUP_SIZE+j][i] = int.from_bytes(payload[(4*(BAROS_GROUP_SIZE*i+j)):(4*(BAROS_GROUP_SIZE*i+j)+4)], ENDIAN, signed=False)/4096
